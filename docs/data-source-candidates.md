...
### Priority 2：短時雨勢預警接入 Decision Explanation Layer — 🟡 暫時擱置
...
**候選代碼調查結果**：
* `F-B0046-001` — 已測試失敗（`Resource not found`）
* `O-A0059-001`（雷達整合回波資料）— 代碼真實存在（`opendata.cwa.gov.tw/index`
  目錄確認，支援 historyAPI），但實測欄位是**經緯度、解析度0.0125°、時間、
  維度(nx*ny)、單位 dBZ**——這是**原始雷達回波格點**，不是預先算好的降雨預報。
  要拿它做「雨勢正在增強還是減弱」，得自己寫網格定位（從 nx*ny 陣列換算嘉義/
  雲林對應的格子）+ 抓多個時間點比較，這正好是當初設計 Rain Cell Tracker 時
  特別提醒要避開的「自己做雷達追蹤」，跟「改用 CWA 已經算好的預報格點」原則
  衝突。使用者確認**暫時擱置**，先去做 Priority 3；O-A0059-001 保留在候選
  清單裡，如果之後找到更合適的預先算好的短時雨量預報資料源，再回頭比較。

⚠️ 註記：O-A0059-001 有提供下載連結（網站上可直接下載，不一定要走 API），
但下載檔案格式（是否為 API 回應相同的 JSON 結構）尚未確認，跟「原始回波 vs
預先算好的預報」這個核心問題無關——即使能下載，資料本質還是原始回波。
...
| 短時雨勢預警（1小時內） | 🟡 暫時擱置 | 需求明確，架構上是**文字修飾層**（跟 CCTV
同一種定位，不碰 Risk Score、不碰排序），不是 Weather Risk Layer 的一部分。
`F-B0046-001` 已測試失敗；`O-A0059-001` 已測試存在但是原始雷達回波（dBZ），
需要自己做網格定位＋趨勢推算，跟「不要自己做雷達追蹤」原則衝突，暫緩，
見下方 Priority 2 段落 |
